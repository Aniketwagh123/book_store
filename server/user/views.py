...
from .task import send_verification_email_task, send_verification_email_task2
...
def verify_registered_user(request, token: str):
    try:
        ...
        if user.is_verified:
            return Response({'message': 'User is already verified', 'status': 'success'}, status=status.HTTP_200_OK)

        user.is_verified = True
        user.save()

        return Response({'message': 'User verified successfully', 'status': 'success'}, status=status.HTTP_200_OK)

    except (ExpiredSignatureError, InvalidTokenError) as e:
        return Response({'message': 'Invalid or expired verification link', 'status': 'error'}, status=status.HTTP_400_BAD_REQUEST)
